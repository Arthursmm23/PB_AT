...
// Função para configurar headers com JWT
function getHeaders() {
    ...
    veiculos.forEach(veiculo => {
        ...
        div.innerHTML = `
            ...
            <div class="mt-3 flex flex-wrap gap-2">
                <button onclick="showEditVeiculoForm('${veiculo.placa}')" 
                        class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded">
                    Editar
                </button>
                <button onclick="deleteVeiculo('${veiculo.placa}')" 
                        class="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded">
                    Excluir
                </button>
                <button onclick="showAddManutencaoForm('${veiculo.placa}')" 
...
// Função para mostrar formulário de adicionar veículo
...
// Função para mostrar formulário de adicionar manutenção
...
async function consultarHistorico() {
    ...
    const tipoConsulta = document.getElementById('historicoTipoConsulta').value;

    try {
        const response = await fetch(`${API_URL}/historico?dataInicial=${dataInicial}&dataFinal=${dataFinal}&tipoConsulta=${tipoConsulta || ''}`, {
            headers: getHeaders()
        });
